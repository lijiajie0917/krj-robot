...
export default {
  ...
  methods:{
    ...
    async getData (currentPage,pageSize,projectId,groupName,conStatus,workStatus) {
      const res = await this.$http.get(`/group/statistics`,{params: {nowpage: currentPage,pagesize:pageSize,projectId:projectId,groupName:groupName,conStatus:conStatus,workStatus:workStatus}}, {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
      const data = res.data
      console.log(data);
      if (data.status === '200') {
